...
// #endif
    ...
    LIBSSH2_POLLFD fds;
    LIBSSH2_LISTENER * ssh2_lsnr = libssh2_channel_forward_listen((LIBSSH2_SESSION*)this->ssh2_sess, 8022);
    qDebug()<<" listened : " <<ssh2_lsnr ;
    //LIBSSH2_CHANNEL *ssh2_channel_forward = libssh2_channel_forward_accept(ssh2_lsnr);
    //qDebug()<<" ssh2 forward new connect  accepted: " << ssh2_channel_forward ;
    int retval = 0;

    while (retval == 0)
    {
        fds.type = LIBSSH2_POLLFD_LISTENER;
        fds.fd.listener = ssh2_lsnr;
        fds.events = LIBSSH2_POLLFD_POLLIN;

        retval = libssh2_poll(&fds,1,500);
