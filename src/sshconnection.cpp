...
int SSHConnection::connect()
{
    ...
    /*
      ...
      DEBUG:  SSHConnection::connect  at  .\sshconnection.cpp  on  150
      conn ping timer started.
      conn ping timer started22222.
      in;Z:\cross\svn-win32-1cro
     */
    // this->mPingTimer->start();
    // q_debug()<<"conn ping timer started.";
    this->connected = true;

    // QTimer::singleShot(3, this, SLOT(listSessionMethords()));
    listSessionMethords();
    // home path
...
int SSHConnection::disconnect()
{
    int rn = 0;
    if (this->sess) {
        rn = libssh2_session_disconnect(this->sess, "Disconnect normal.");
        if (rn < 0) {
            q_debug()<<"disconnect error";
        }
        rn = libssh2_session_free(this->sess);
        if (rn < 0) {
            q_debug()<<"session free error";
        }
        this->sess = NULL;
    }
...
int SSHConnection::listSessionMethords()
{
    q_debug()<<""<<sender();
    const static char *smethods[] = {
        "LIBSSH2_METHOD_KEX", "LIBSSH2_METHOD_HOSTKEY", "LIBSSH2_METHOD_CRYPT_CS", "LIBSSH2_METHOD_CRYPT_SC",
        "LIBSSH2_METHOD_MAC_CS", "LIBSSH2_METHOD_MAC_SC", "LIBSSH2_METHOD_COMP_CS", "LIBSSH2_METHOD_COMP_SC", 
    ...
    for (int i = 0; imethod[i] != -1; i++) {
        method = libssh2_session_methods(this->sess, imethod[i]);
        q_debug()<<"method:"<<i<<QString(smethods[i])<<QString(method);
    }
