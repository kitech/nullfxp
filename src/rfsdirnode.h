...
public:
    ...
    /*
      0x01 表示UI没有要求过的，所以我们也没有取过数据的            
      0x02 表示UI没有请求过，我们自己填入了部分数据（如用户指定他的初始化目录的时候），即这表示需要更新的结点
      0x04 表示UI已经请求过，我们只取回来了一部分数据，即这表示需要立即更新的结点
      0x08 表示UI已经请求过，我们已经放到处理队列的，但还没有获取来数据的
      0x10 表示UI已经请求过，并且更新到了最新目录结构状态。
