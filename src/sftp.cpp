...
int showprogress = 0 ;   //showprogress = 0;////////////////////////bug 
...
//add by liuguangzhao , is that ok ?????
//static 
int
...
/*
 这个函数使用 -S 指定的SSh命令（如/usr/bin/ssh ，或者默认的命令 ssh 连接到远程主机的sftp-server。
 同时创建与当前进程通信的 文件描述字。
 此SSh是fork出来的进程，使用socketpair将ssh进程的标准输入输出连接到当前sftp进程的标准输入和输出。
*/
...
sftp_main(int argc, char **argv)
{
	...
	freeargs(&args);
        ...
        //file1 和 file2 是不是分别为本地路径和远程路径呢？
